...
def net(num_classes = 133):
    ...
    # Reemplazamos la capa totalmente conectada (FC)
...
def create_data_loaders(data_path, batch_size):
    """
    Crea los loaders. Note que ImageFolder detecta subcarpetas como clases.
    """
...
def main(args):
    ...
    print(f"Executing HPO script in: {device}")
    ...
    '''
    train function to start training your model
    Remember that you will need to set up a way to get training data from S3
    '''
    start_time = time.time()
    train(model, train_loader, valid_loader, loss_criterion, optimizer, args.epochs, args.model_dir, device)
    ...
    # Guardar el modelo resultante del job de tuning
    # torch.save(model.state_dict(), os.path.join(args.model_dir, "dog_breed_clsfr_model.pth"))
    '''
